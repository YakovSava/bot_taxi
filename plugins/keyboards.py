...
class keyboards:
	...
	driver_order_complete = lambda link: (Keyboard(one_time = False)
		.add(Text('Буду через 5 минуты!', payload = {'driver': 0, 'minute': 5, 'other': link}), color = KeyboardButtonColor.PRIMARY)
		.row()
		.add(Text('Буду через 10 минут!', payload = {'driver': 0, 'minute': 10, 'other': link}), color = KeyboardButtonColor.PRIMARY)
		.row()
		.add(Text('Буду через 15 минут!', payload = {'driver': 0, 'minute': 15, 'other': link}), color = KeyboardButtonColor.PRIMARY)
		.row()
	...
	promo = lambda driver: (Keyboard()
		...
		.row()
		.add('Назад', payload=({'driver': 0, 'profie': 0} if driver else {'user': 0, 'back': 0}), color=KeyboardButtonColor.NEGATIVE)
	).get_json()
	promo_back = lambda driver: (Keyboard()
		.add('Назад', payload=({'driver': 0, 'profie': 0} if driver else {'user': 0, 'back': 0}), color=KeyboardButtonColor.PRIMARY)
	).get_json()
