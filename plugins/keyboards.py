...
class keyboards:
	...
	class inline:
		...
		yoomoney_check_pay = lambda x: (Keyboard(one_time = False, inline = True)
			.add(Text('Проверить оплату', payload={'driver': 0, 'yoomoney': 0, 'other': x}), color=KeyboardButtonColor.PRIMARY)
		).get_json()
		...
		sudo_delete = (Keyboard(inline=True)
			.add(Text('Да удалить', payload={'sudo': 1, 'delete': 0}), color=KeyboardButtonColor.POSITIVE)
			.row()
			.add(Text('Отмена', payload={'driver': 0, 'profie': 0}), color=KeyboardButtonColor.NEGATIVE)
		).get_json()
	...
	payeer = (Keyboard(one_time=False)
		# .add(Text('Пополнить при помощи VK PAY', payload={'driver': 0, 'money': 'vk pay'}), color=KeyboardButtonColor.POSITIVE)
		# .row()
		# .add(Text('Пополнить при помощи Yoomoney', payload={'driver': 0, 'money': 'yoomoney'}), color=KeyboardButtonColor.PRIMARY)
		# .row()
		#.add(Text('Пополнить при помощи QIWI', payload={'driver': 0, 'money': 'qiwi'}), color=KeyboardButtonColor.POSITIVE)
		#.row()
		.add(Text('Пополнить с помощью карты (Yoomoney)', payload={'driver': 0, 'money': 'yoomoney'}),
			color=KeyboardButtonColor.POSITIVE)
		.row()
		.add(Text('Назад', payload={'driver': 0, 'profie': 0}), color=KeyboardButtonColor.NEGATIVE)
	).get_json()
